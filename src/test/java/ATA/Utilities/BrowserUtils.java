...
public class BrowserUtils{
    ...
    public static String getScreenShotZoom(String name){
        zoomInOut(Driver.getDriver(),0.50);
        String ssPath = getScreenshot(name);
        zoomInOut(Driver.getDriver(),1.00);
        return ssPath;
    }

    public static void zoomInOut(WebDriver driver, double scale) {
    ...
    public static void scrollDown(int num1, int num2, int num3, int num4) {
        JavascriptExecutor j = (JavascriptExecutor) Driver.getDriver();
        j.executeScript("window.scrollBy(" + num1 + "," + num2 + ")");
        j.executeScript("window.scrollBy(" + num3 + "," + num4 + ")");
    }
